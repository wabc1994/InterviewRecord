...
- 一个很关键的变量 SZIECTL 关键变量，这个变量直接声明为 private static int 类型，主要用来控制真个table，主要通过Unsafe,当中的CAS操作
...
- TreeBin，要知道我们将链表转换成为红黑树的时候，是将整个东西转化为红黑树，从头结点开始到尾巴结点，而不是在同一个桶当中红黑树和链表结合的方式
